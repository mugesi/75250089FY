...
- RMSEA indicates a good model fit, i.e. a 4-factor solution does not seem to be a bad idea. (we use CFI and TLI rather for comparison between models, see lecture 6).
- All four factors have eigenvalues greater than 1 (This is not a strict requirement in EFA, but good to check).
- All factors have more than two meaningful items that load strongly on them, and items that load strongly on one factor and do not load strongly on other factors also make sense (This is easier to see from the diagram in step 8. Always check what the items stand for).
