...
# Week 5 Exercises 
In this set of exercises we will use the data set "SAQ.sav". This dataset includes 23 variables measured on a 5-point Likert scale and contains data from 2,571 participants. The items included are:
...
# - RMSEA indicates a good model fit, i.e. a 4-factor solution does not seem to be a bad idea. (we use CFI and TLI rather for comparison between models).
# - All four factors have eigenvalues greater than 1 (This is not a strict requirement in EFA, but good to check).
# -All factors have more than two meaningful items that load strongly on them. Items that load strongly on one factor while not loading strongly on others also make sense (always check what the items represent). However, item complexity is not good, as many items load well on more than one factor.  
```
...
#We have 253 residuals from a 23*23 matrix, and only 26 of them are greater than 0.05. That may be acceptable. Another solution would be to perform another factor analysis with a 5-factor solution and compare the results.

#Overall, the model fit and residuals appear acceptable. However, the total explained variance of 40% is relatively low. More importantly, several items show substantial cross-loadings on multiple factors, which raises concerns about the clarity of the factor structure.
```
